...
export const Button = ({
  ...
  return (
    <button
      type={type}
      className={cn(button({ intent, variant, size, rounded, disabled, loading }), className)}
      disabled={disabled}
      {...props}
    >
      {children}
...
const button = cva(['font-semibold rounded inline-flex items-center justify-center transition'], {
  variants: {
    intent: {
  ...
  compoundVariants: [
    ...
    },
    {
      intent: 'danger',
      variant: 'outline',
      class: ['bg-danger-1 border-danger-7 text-danger-hi hover:border-danger-8 hover:bg-danger-1'],
    },
    {
      intent: 'success',
      variant: 'outline',
